...
let REFRESH_TOKEN =
  "1//0gch2kfOrE_s7CgYIARAAGBASNwF-L9Ir7WDclt_84IW6Re73UvovudCzj1WgMVOVwEqdu0hswHtA0SjHzvl0uI5L3eOU2DjTaoc";
...
app.get("/google/redirect", async (req, res) => {
  ...
  // setting the refresh token here
...
app.get("/get_google_events", async (req, res) => {
  oauth2Client.setCredentials({ refresh_token: REFRESH_TOKEN });
  try {
...
app.get("/update_google_event", async (req, res) => {
  oauth2Client.setCredentials({ refresh_token: REFRESH_TOKEN });
  try {
    const event = { summary: "updated summery text!!!" };

    return calendar.events.patch(
      {
        auth: oauth2Client,
        calendarId: "primary",
        eventId: "4blf5jml19prqso7mjh7ak8nn3",
        resource: event,
      },
      function (err, event) {
        if (err) {
        ...
        return res.json({ message: "Event updated: %s", data: event });
      }
    );
  } catch (err) {
    return res.json({ err });
